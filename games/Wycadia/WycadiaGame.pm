...
sub WycadiaGame {
	...
	$self->{room} = Wycadia::MapRoomMorningGlory->MapRoomMorningGlory($self->{screen});	
	$self->{Aldhebrand} = Wycadia::Aldhebrand->Aldhebrand($self->{screen});	
...
###	my $gameover = 0;
	...
	while (1) {
		...
		if ($event->type() == SDL_KEYDOWN) {
			...
			###	$self->{Aldhebrand}->move(-1,0,0);
			}	
			if ($event->key_sym == SDLK_RIGHT) {
			###	$self->{Aldhebrand}->move(1,0,0);
			}	
			if ($event->key_sym == SDLK_UP) {
			###	$self->{Aldhebrand}->move(0,-1,0);
			}	
